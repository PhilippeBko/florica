...
-----------------------------------------------------------------------------------------------------
ALTER TABLE taxonomy.taxa_nameset DROP COLUMN IF EXISTS _keyname;
ALTER TABLE taxonomy.taxa_nameset
ADD COLUMN _keyname TEXT GENERATED ALWAYS AS (taxonomy.pn_taxa_keyname(name)) STORED;
...
CREATE INDEX ON taxonomy.taxa_nameset USING gin (_keyname gin_trgm_ops);
...
BEGIN
	...
	SELECT 
		CASE 
			WHEN tocategory = 'homotypic' THEN 5
			WHEN tocategory = 'heterotypic' THEN 6
			WHEN tocategory = 'vernacular' THEN 7
			WHEN tocategory = 'orthographic' THEN 8
			WHEN tocategory = 'basionym' THEN 9
			WHEN tocategory = 'informal' THEN 10
			WHEN tocategory = 'misapplied' THEN 11
		ELSE 12
		END
	INTO newcategory;
	IF newcategory = 9 THEN
		UPDATE taxonomy.taxa_nameset
		SET category = 5
		WHERE id_taxonref = idtaxonref and category = 9;
	END IF;
...
BEGIN
	IF newcategory IS NOT NULL THEN
		...
		SELECT 
			CASE 
				WHEN newcategory = 'homotypic' THEN 5
				WHEN newcategory = 'heterotypic' THEN 6
				WHEN newcategory = 'vernacular' THEN 7
				WHEN newcategory = 'orthographic' THEN 8
				WHEN newcategory = 'basionym' THEN 9
				WHEN newcategory = 'informal' THEN 10
				WHEN newcategory = 'misapplied' THEN 11
			ELSE 12
			END
		INTO _category;

		IF _category = 9 THEN
			SELECT id_taxonref INTO _idtaxonref FROM taxonomy.taxa_nameset WHERE name = nametoupdate;
			UPDATE taxonomy.taxa_nameset
			SET category = 5
			WHERE id_taxonref = _idtaxonref and category = 9;
		END IF;
	...
	SELECT 
		a.name,  
			CASE 
				...
				WHEN a.category = 5 THEN 'Homotypic'
				WHEN a.category = 6 THEN 'Heterotypic'
				WHEN a.category = 7 THEN 'Vernacular'
				WHEN a.category = 8 THEN 'Orthographic'
				WHEN a.category = 9 THEN 'Basionym'
				WHEN a.category = 10 THEN 'Informal'
				WHEN a.category = 11 THEN 'Misapplied'
			ELSE 'Unknown'
			END
			AS _category,
			a.category
 ...
 */
    SELECT rank_name
    FROM taxonomy.taxa_rank
    WHERE id_rank = idrank;
$function$;
...
RETURN QUERY
    ...
    WITH 
		...
		taxa_others AS (--generate Concatenated name for infra-families taxa (ex: create taxa "Fabaceae - subfam. Mimosoideae" FROM "Mimosoideae")
				SELECT  a.id_taxonref, concat_ws (chr(32), 'Plantae', d.prefix, INITCAP(a.basename)) AS taxa_name, a.authors
				FROM tmp_taxa_reference a
...
CREATE OR REPLACE PROCEDURE taxonomy.pn_taxa_set_synonymy(IN from_idtaxonref integer, IN to_idtaxonref integer, IN cat_synonym text DEFAULT 'Homotypic'::text)
 LANGUAGE plpgsql
AS $procedure$
/*
	call taxonomy.pn_taxa_set_synonymy (16875, 1577,'')
	call taxonomy.pn_taxa_set_synonymy (13570, 16609,'Heterotypic')
*/
DECLARE
	_idparent	INTEGER;
	from_idrank	INTEGER;
...
--DO UPDATE
	--update id_taxonref and category for names, set category for autogenerated names (category < 5)
		SELECT 4 + array_position(ARRAY['Homotypic', 'Heterotypic'], cat_synonym) INTO _idcategory;
...
COMMENT ON VIEW taxonomy.taxa_hierarchy IS 'View to display hierarchy for any taxa in taxa_reference'
;
